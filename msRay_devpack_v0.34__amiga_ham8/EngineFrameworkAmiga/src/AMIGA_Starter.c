...
UBYTE STA_Get_Screen_Modes()
{
 ...
 if (GetDisplayInfoData(NULL,  (UBYTE *) &nameInfo,   sizeof(struct NameInfo),   DTAG_NAME,   modeID))
  {
             GetDisplayInfoData(NULL,   (UBYTE *) &displayinfo,   sizeof(struct DisplayInfo),   DTAG_DISP,  modeID);
 			...
 			if (!(displayinfo.NotAvailable)) 
 			{
                                        ...
                                        GetDisplayInfoData(NULL,  (UBYTE *) &dn_Dimensioninfo,   sizeof(struct DimensionInfo),  DTAG_DIMS,  modeID);
				...
				//	GetDisplayInfoData (display_info_handler, (APTR)&nameInfo, sizeof (struct NameInfo), DTAG_NAME, INVALID_ID);
					...
					sprintf(buffer_3, "%s", "x" );	

					char buffer_2[16];
					memset(buffer_2, 0, 16);
					sprintf(buffer_2, "%d x %d", dn_Dimensioninfo.Nominal.MaxX+1, dn_Dimensioninfo.Nominal.MaxY+1);

					char buffer_1[64];
					memset(buffer_1, 0, 64);		
					sprintf(buffer_1, "%s:   %d x %d      ", buffer_driver, dn_Dimensioninfo.Nominal.MaxX+1, dn_Dimensioninfo.Nominal.MaxY+1 );
