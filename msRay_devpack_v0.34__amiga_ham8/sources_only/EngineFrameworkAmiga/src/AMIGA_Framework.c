...
const int bplsize = FRM_requested_width * FRM_requested_height / 8;
const int chunkylen = 320 * 4;
const int depth = 8;

IO_prefs.output_buffer_32 = (u_int32*)malloc(320 * 256*4);

//IO_prefs.output_buffer_8 = (u_int8*)malloc(FRM_requested_width * FRM_requested_height);

//u_int32* fast_buffer32 = (u_int32*)malloc(320*256*4);

UBYTE* chip_buffer = (UBYTE*)AllocMem(bplsize * depth, MEMF_CHIP); // <-- bplsize * depth = (320*256/8) * 8
...
memset(FRM_mbuf_screen_buffer[1]->sb_BitMap->Planes[6], 0x77, bplsize);
memset(FRM_mbuf_screen_buffer[1]->sb_BitMap->Planes[7], 0xcc, bplsize);

/*
FRM_screen->BitMap.Planes[0] = (PLANEPTR)chip_buffer + bplsize * 0;
FRM_screen->BitMap.Planes[1] = (PLANEPTR)chip_buffer + bplsize * 1;
FRM_screen->BitMap.Planes[2] = (PLANEPTR)chip_buffer + bplsize * 2;
FRM_screen->BitMap.Planes[3] = (PLANEPTR)chip_buffer + bplsize * 3;
FRM_screen->BitMap.Planes[4] = (PLANEPTR)chip_buffer + bplsize * 4;
FRM_screen->BitMap.Planes[5] = (PLANEPTR)chip_buffer + bplsize * 5;
FRM_screen->BitMap.Planes[6] = (PLANEPTR)chip_buffer + bplsize * 6;
FRM_screen->BitMap.Planes[7] = (PLANEPTR)chip_buffer + bplsize * 7;*/
...
//c2p1x1_8_c5_040_init(chunkyx, chunkyy, scroffsy, bplsize);
 ...
 // get fps
                             ...
                             //   {	        
                                  ...
                                  //  c2p1x1_8_c5_040(IO_prefs.output_buffer_8, chip_buffer);
    ...
    signal_mask =  1 << FRM_window_user_port->mp_SigBit;
...
//FRM_requested_display_id = PAL_MONITOR_ID | HAM_KEY									;

struct DimensionInfo dn_Dimensioninfo;

	    
     GetDisplayInfoData(NULL,  (UBYTE *) &dn_Dimensioninfo,   sizeof(struct DimensionInfo),  DTAG_DIMS,  FRM_requested_display_id);

FRM_requested_width = dn_Dimensioninfo.Nominal.MaxX+1;
FRM_requested_height = dn_Dimensioninfo.Nominal.MaxY+1;

 FRM_requested_pixel_depth = dn_Dimensioninfo.MaxDepth;
...
UBYTE FRM_Init_Screen()
{
    ...
    FRM_screen = OpenScreenTags(	NULL,
                                    ...
                                    SA_Depth, FRM_requested_pixel_depth, 						
