...
void GUI_Update_Properties_Walls(HWND _hwnd)
{
    switch (gui_current_list_index)
    {
        ...
        case LIST_WALL_THIN_HORIZONTAL:  
            ...
            if ((MAP_working_cell.wall_vertex[0][0] > -0.0001f && MAP_working_cell.wall_vertex[0][0] < 0.0001f &&
                  MAP_working_cell.wall_vertex[0][1] > -0.0001f && MAP_working_cell.wall_vertex[0][1] < 0.0001f &&
                  MAP_working_cell.wall_vertex[1][0] > -0.0001f && MAP_working_cell.wall_vertex[1][0] < 0.0001f &&
                  MAP_working_cell.wall_vertex[1][1] > -0.0001f && MAP_working_cell.wall_vertex[1][1] < 0.0001f) ||
    ...
    switch (gui_current_list_index)
    {
        ...
        case LIST_DOOR_BOX_FOURSIDE_VERTICAL_RIGHT:
            ...
            if ((MAP_working_cell.wall_vertex[0][0] > -0.0001f && MAP_working_cell.wall_vertex[0][0] < 0.0001f &&
                ...
                MAP_working_cell.wall_vertex[0][0] == MAP_working_cell.wall_vertex[1][0] ||
                MAP_working_cell.wall_vertex[0][1] == MAP_working_cell.wall_vertex[1][1] ||
                APP_level_file.map[selected_cell_index].cell_type != LIST_DOOR_BOX_FOURSIDE_VERTICAL_LEFT ||
                APP_level_file.map[selected_cell_index].cell_type != LIST_DOOR_BOX_FOURSIDE_VERTICAL_RIGHT)
            {
                MAP_working_cell.wall_vertex[0][0] = 0.2f;
                MAP_working_cell.wall_vertex[0][1] = 0.0f;
                MAP_working_cell.wall_vertex[1][0] = 0.8f;
...
void    TEX_Prepare_List_To_Save(std::list<s_Texture>* _list, int* _table, int _mode)
{
    ...
    // The function cleanup list from unused textures, sort them in A-Z order,
    // and put the texture names in level file.
    ...
    if (_mode == 0)
    {
        if (strlen(APP_level_file.wall_texture_filename[0]) == 0)
            strcpy(APP_level_file.wall_texture_filename[0], "_default.tw");
    }
    else
    {
...
void    TEX_Count_Textures()
{
    ...
    for (int i = 0; i < LV_MAP_CELLS_COUNT; i++)
    {
        ...
        TEX_flat_table[APP_level_file.map[i].flat_texture_id[id_floor]]++;
        TEX_flat_table[APP_level_file.map[i].flat_texture_id[id_ceil]]++;
    }
