    ...
    switch (_message)
    {
        ...
        case WM_COMMAND:
        {
            ...
            switch (wm_id)
            {
                ...
                // --- properties ---
                    ...
                    case IDB_PROP_GENERATE_FV:
                        APP_Generate_Flats_Visibility_Lists();
                        GUI_Update_Resources(_hwnd);
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;

                    case IDB_PROP_FV_SB1:
                        gui_selected_fv_list = 0;
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;

                    case IDB_PROP_FV_SB2:
                        gui_selected_fv_list = 1;
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;

                    case IDB_PROP_FV_SB3:
                        gui_selected_fv_list = 2;
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;

                    case IDB_PROP_FV_SB4:
                        gui_selected_fv_list = 3;
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;

                    case IDB_PROP_FV_SB5:
                        gui_selected_fv_list = 4;
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;

                    case IDB_PROP_FV_SB6:
                        gui_selected_fv_list = 5;
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;

                    case IDB_PROP_FV_SB7:
                        gui_selected_fv_list = 6;
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;

                    case IDB_PROP_FV_SB8:                            
                        gui_selected_fv_list = 7;
                        RedrawWindow(_hwnd, NULL, NULL, RDW_INVALIDATE);
                        break;
        ...
        case WM_PAINT:
        {
            ...
            HDC hdc_map = BeginPaint(GetDlgItem(_hwnd, IDS_MAP), &ps_map);
                ...
                Map_Draw_Objects_Flats_Visibility_List(hdc_tmp_buffer);
...
int APP_Open()
{
    ...
    if (GUI_OpenDialog(APP_filename, APP_Open_FILE, APP_MAP_FILTER, APP_DEFEXT_MAP))
    {
        ...
        else
        {
            ...
            // make swaps
            APP_level_file.fv_list_size = MA_swap_int32(APP_level_file.fv_list_size);
            ...
            for (int cell = 0; cell < LV_MAP_CELLS_COUNT; cell++)
            {
                if (APP_level_file.map[cell].is_fv_marked)
                {
                    for (int slice = 0; slice < 8; slice++)
                    {
                        APP_level_file.map[cell].fv_list[slice] = MA_swap_int16(APP_level_file.map[cell].fv_list[slice]);
                    }  
                } 
            }

            // now read in the dynamic data
            
            // alloc mem for fv list and read in
            int16* tmp_fv_list = (int16*)malloc(APP_level_file.fv_list_size);
            fread(tmp_fv_list, APP_level_file.fv_list_size, 1, file_in);
            ...
            // put fv list from tmp buffer to editor lists
            int offset = 0;

            for (int i = 0; i < LV_MAP_CELLS_COUNT; i++)
            {
                if (APP_level_file.map[i].is_fv_marked)
                {
                    for (int slice = 0; slice < 8; slice++)
                    {
                        for (int p = 0; p < APP_level_file.map[i].fv_list[slice]; p++)
                        {
                            MAP_fv_list[i][slice].push_back( MA_swap_int16( tmp_fv_list[offset] ) );
                            offset++;
                        }
                    }
                }
...
void APP_Save()
{
    ...
    else
    {
        ...
        // Swap endiannes for 16 and 32 bit values before saving
            ...
            for (int i = 0; i < LV_MAP_CELLS_COUNT; i++)
            {
                ...
                if (APP_level_file.map[i].is_fv_marked)
                {
                    for (int slice = 0; slice < 8; slice++)
                    {
                        APP_level_file.map[i].fv_list[slice] = MA_swap_int16(APP_level_file.map[i].fv_list[slice]);
                    }
                }
            }
        ...
        // try save structure (the header) to file
        ...
        // now try to save dynamic data after the header - the lists with potentially visible cells.
            int result2 = 0;
            ...
            for (int i = 0; i < LV_MAP_CELLS_COUNT; i++)
            {
                if (APP_level_file.map[i].is_fv_marked)
                {
                    for (int r = 0; r < 8; r++)
                    {
                        for (auto it = MAP_fv_list[i][r].begin(); it != MAP_fv_list[i][r].end(); it++)
                        {
                            int16 value = MA_swap_int16(*it);
                            result2 = (int)fwrite(&value, 2, 1, file_out);
                        }                    
                    }
                }             
            }
            ...
            if (!result2)
                MessageBox(APP_hWnd, L"Could not save dynamic data to file...", L"Info...", MB_OK);
...
{
    // 'Brute Force' algorithm for generating list of potentially visible cells for each cell that is divided into eight 45* regions.

    // 1. Go thru the map and for each cell that player can move into (marked as 'is_fv_marked') do the calculations...
    // 2. Divide cell into 8 pieces/slices - each 45*: (0-45)(45-90)(90-135)(135-180)(180-225)(225-270)(270-315)(315-360)
    // 3. For every slice we must set player in different positions and angles to cover most of the possible move/view variations
    // 3a. Set starting x,y positions that will simulate player position. The staring point is top-left corner.
    // 3b. make loops that will go thru the current cell in x and y by some step, maybe 0.2f
    // 3c. for every position in step 3, me must also simulate player lookaroud in 45*. So we also will rotate by some step like 45/5 = 9 units.
    //     The rotate range depends on current slice.
    // 4. For each pair of (x,y) and current rotate step run a Raycast algorithm for example for width = 640px. Add to proper list MAP_fv_list[current_cell_id][current_slice]
    //    all the found cells that are not walls (only that one that are creating floor/ceil tiles)

    // clear info in level structure
    for (int i = 0; i < LV_MAP_CELLS_COUNT; i++)
    {
        // 16 = 8 lists * 2 bytes
        memset(APP_level_file.map[i].fv_list, 0, 16);
        
        MAP_fv_list[i][0].clear();
        MAP_fv_list[i][1].clear();
        MAP_fv_list[i][2].clear();
        MAP_fv_list[i][3].clear();
        MAP_fv_list[i][4].clear();
        MAP_fv_list[i][5].clear();
        MAP_fv_list[i][6].clear();
        MAP_fv_list[i][7].clear();
    }

    // virtual player move and rotation steps (degrees)
    // -- decrease for more precision
    const float p_mstep = 0.2f;
    const float p_rstep = 9.0f;

    // raycast constans
    // -- increase for more precision
    const int raycast_width = 1920;
    const int raycast_height = 1200;

    const float render_aspect = (float)raycast_width / (float)raycast_height;

    APP_level_file.fv_list_size = 0;

    // ...for every map cell in a column...
    for (int cy = 0; cy < LV_MAP_LENGTH; cy++)
    {
        // ... for every map cell in a row...
        for (int cx = 0; cx < LV_MAP_LENGTH; cx++)
        {
            int cell_id = cx + cy * LV_MAP_LENGTH;

            // ...if that cell is marked as flat_visible...
            if (APP_level_file.map[cell_id].is_fv_marked)
            {
                // ... divide that cell into 8 pieces/slices - each by 45*...
                for (int slice = 0; slice < 8; slice++)
                {
                    // at first add the cell that players stands on..
                    MAP_fv_list[cell_id][slice].push_back(cell_id);

                    // set slice start and end degree
                    float slice_start_deg = slice * 45.0f;
                    float slice_end_deg = slice_start_deg + 45.0f;

                    // ...for every next step in cell column (0.0-0.8)...
                    for (float sy = 0.0f; sy <= 1.0f; sy += p_mstep)
                    {
                        // ...for every next step in cell row (0.0-0.8)...
                        for (float sx = 0.0f; sx <= 1.0f; sx += p_mstep)
                        {
                            // ...for each step in 45* player view range...
                            for (float p_angle = slice_start_deg; p_angle <= slice_end_deg; p_angle += p_rstep)
                            {
                                float pp_dx = 0.0f;
                                float pp_dy = -1.0f;
                                float pp_nsize_x__div2 = render_aspect / 2.0f;
                                float pp_nsize_y__div2 = 0.0f;

                                // convert p_angle from degrees to radians
                                float p_angle_rad = (p_angle * MA_pi) / 180.0f;

                                // ..
                                // The raycaster algorithm is the same as in the game renderer - a bit simplified
                                // ..

                                // update raycaster starting inputs for that current case
                                float tmp_sinf = sinf(p_angle_rad);
                                float tmp_cosf = cosf(p_angle_rad);

                                float old_pp_dx = pp_dx;
                                pp_dx = pp_dx * tmp_cosf - pp_dy * tmp_sinf;
                                pp_dy = old_pp_dx * tmp_sinf + pp_dy * tmp_cosf;

                                float old_pp_nsize_x = pp_nsize_x__div2;
                                pp_nsize_x__div2 = pp_nsize_x__div2 * tmp_cosf - pp_nsize_y__div2 * tmp_sinf;
                                pp_nsize_y__div2 = old_pp_nsize_x * tmp_sinf + pp_nsize_y__div2 * tmp_cosf;

                                // Lets prepare some variables before entering main loop.
                                int player_x_integer = cx;
                                int player_y_integer = cy;

                                float player_x_fraction = sx;
                                float minus__player_x_fraction__add__one = -player_x_fraction + 1.0f;

                                float player_y_fraction = sy;
                                float minus__player_y_fraction__add__one = -player_y_fraction + 1.0f;

                                float side_dist_x, side_dist_y;
                                int step_x = 0, step_y = 0;

                                int ray_x_counter = raycast_width;
                                int ray_x = -1;

                                // run raycast algorith for raycast_width
                                while (ray_x_counter)
                                {
                                    ray_x_counter--;
                                    ray_x++;

                                    float pp_coord_x = 2.0f * ray_x / raycast_width - 1;

                                    float ray_dir_x = pp_dx + pp_nsize_x__div2 * pp_coord_x;
                                    float ray_dir_y = pp_dy + pp_nsize_y__div2 * pp_coord_x;

                                    float ray_dir_x__1div = 1.0f / ray_dir_x;
                                    float ray_dir_y__1div = 1.0f / ray_dir_y;

                                    float delta_dist_x = fabsf(ray_dir_x__1div);
                                    float delta_dist_y = fabsf(ray_dir_y__1div);

                                    int map_x = player_x_integer;
                                    int map_y = player_y_integer;
                                    ...
                                    while (is_solid_hit == 0)
                                    {
                                        ...
                                        int hit_map_id = map_x + (map_y << LV_MAP_LENGTH_BITSHIFT);

                                        // Check if ray has hit a wall...
                                        if (APP_level_file.map[hit_map_id].is_cell_solid)
                                        {
                                        ...
                                        {
                                            // if not solid wall - add hit_map_id to list as floor/ceil tile..
                                            auto it = std::find(MAP_fv_list[cell_id][slice].begin(), MAP_fv_list[cell_id][slice].end(), hit_map_id);
                                            
                                            if (it == MAP_fv_list[cell_id][slice].end())
                                            {
                                                MAP_fv_list[cell_id][slice].push_back(hit_map_id);
                                            }
                                        }
                                    }
                                }
                    ...
                    // At the end of each slice put info about numer of elements.
                    
                    MAP_fv_list[cell_id][slice].sort();
                    APP_level_file.map[cell_id].fv_list[slice] = MAP_fv_list[cell_id][slice].size();
                    APP_level_file.fv_list_size += MAP_fv_list[cell_id][slice].size();
                }
            }
        }
    }

    // at the end multiply sum of all entries by 2, because entries in the list are from 0-4095 so will take 2 bytes in memory.
    APP_level_file.fv_list_size *= 2;
}
...
void GUI_Create(HWND _hwnd)
{
    ...
    hp_flats_visibility_grid = CreatePen(PS_SOLID, 1, RGB(25, 22, 0));
    ...
    hp_flats_visibility_dotted = CreatePen(PS_DOT, 0, RGB(193, 183, 70));
    ...
    hbr_map_flats_visibility = CreateSolidBrush(RGB(92, 81, 0));
    hbr_map_flats_visibility_list = CreateSolidBrush(RGB(110, 8, 8));
    ...
    hbr_flats_visibility_bg = CreateSolidBrush(RGB(38, 33, 0));
    ...
    // --- MEMORY INFO ----
        ...
        CreateWindowEx( 0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_ETCHEDFRAME,
                        mem_groupbox_x, mem_groupbox_y, 210, 200, _hwnd, (HMENU)-1, GetModuleHandle(NULL), nullptr);
        ...
        CreateWindowEx(0, L"STATIC", L"File header size:", WS_CHILD | WS_VISIBLE | SS_LEFT,
                         mem_groupbox_x + 10, mem_groupbox_y + 18, 100, 20, _hwnd, (HMENU)IDS_RES_FILEH_LABEL, GetModuleHandle(NULL), nullptr);

        CreateWindowEx(0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_RIGHT,
                        mem_groupbox_x + 110, mem_groupbox_y + 18, 90, 20, _hwnd, (HMENU)IDS_RES_FILEH, GetModuleHandle(NULL), nullptr);


        CreateWindowEx(0, L"STATIC", L"File data size:", WS_CHILD | WS_VISIBLE | SS_LEFT,
                        mem_groupbox_x + 10, mem_groupbox_y + 36, 100, 20, _hwnd, (HMENU)IDS_RES_FILED_LABEL, GetModuleHandle(NULL), nullptr);

        CreateWindowEx(0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_RIGHT,
                        mem_groupbox_x + 110, mem_groupbox_y + 36, 90, 20, _hwnd, (HMENU)IDS_RES_FILED, GetModuleHandle(NULL), nullptr);


        CreateWindowEx( 0, L"STATIC", L"Wall textures:", WS_CHILD | WS_VISIBLE | SS_LEFT,
                        mem_groupbox_x + 10, mem_groupbox_y + 58, 90, 20, _hwnd, (HMENU)IDS_RES_WALL_TEX_LABEL, GetModuleHandle(NULL), nullptr);

        CreateWindowEx( 0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_RIGHT,
                        mem_groupbox_x + 110, mem_groupbox_y + 58, 90, 20, _hwnd, (HMENU)IDS_RES_WALL_TEX, GetModuleHandle(NULL), nullptr);


        CreateWindowEx( 0, L"STATIC", L"Flat textures:", WS_CHILD | WS_VISIBLE | SS_LEFT,
                        mem_groupbox_x + 10, mem_groupbox_y + 76, 90, 20, _hwnd, (HMENU)IDS_RES_FLAT_TEX_LABEL, GetModuleHandle(NULL), nullptr);

        CreateWindowEx( 0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_RIGHT,
                        mem_groupbox_x + 110, mem_groupbox_y + 76, 90, 20, _hwnd, (HMENU)IDS_RES_FLAT_TEX, GetModuleHandle(NULL), nullptr);


        CreateWindowEx( 0, L"STATIC", L"Wall lightmaps:", WS_CHILD | WS_VISIBLE | SS_LEFT,
                        mem_groupbox_x + 10, mem_groupbox_y + 98, 90, 20, _hwnd, (HMENU)IDS_RES_W_LIGHTMAPS_LABEL, GetModuleHandle(NULL), nullptr);

        CreateWindowEx( 0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_RIGHT,
                        mem_groupbox_x + 110, mem_groupbox_y + 98, 90, 20, _hwnd, (HMENU)IDS_RES_W_LIGHTMAPS, GetModuleHandle(NULL), nullptr);


        CreateWindowEx(0, L"STATIC", L"Flat lightmaps:", WS_CHILD | WS_VISIBLE | SS_LEFT,
                        mem_groupbox_x + 10, mem_groupbox_y + 116, 90, 20, _hwnd, (HMENU)IDS_RES_F_LIGHTMAPS_LABEL, GetModuleHandle(NULL), nullptr);

        CreateWindowEx(0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_RIGHT,
                        mem_groupbox_x + 110, mem_groupbox_y + 116, 90, 20, _hwnd, (HMENU)IDS_RES_F_LIGHTMAPS, GetModuleHandle(NULL), nullptr);


        CreateWindowEx( 0, L"STATIC", L"Total:", WS_CHILD | WS_VISIBLE | SS_LEFT,
                        mem_groupbox_x + 10, mem_groupbox_y + 138, 90, 20, _hwnd, (HMENU)IDS_RES_TOTAL_LABEL, GetModuleHandle(NULL), nullptr);

        CreateWindowEx( 0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_RIGHT,
                        mem_groupbox_x + 110, mem_groupbox_y + 138, 90, 20, _hwnd, (HMENU)IDS_RES_TOTAL, GetModuleHandle(NULL), nullptr);

        // add refresh button

        CreateWindowEx(0, L"BUTTON", L"Refresh", WS_CHILD | WS_VISIBLE,
                        mem_groupbox_x + 8, mem_groupbox_y + 168, 193, 22, _hwnd, (HMENU)IDB_RES_REFRESH, GetModuleHandle(NULL), NULL);
    ...
    // ---- LAYERS ----
        ...
        int layers_groupbox_x = 980, layers_groupbox_y = 223;
        ...
        SendMessage(GetDlgItem(_hwnd, IDC_LAYERS_COMBO), (UINT)CB_ADDSTRING, (WPARAM)0, (LPARAM)L"05. Flats visibility");

        SendMessage(GetDlgItem(_hwnd, IDC_LAYERS_COMBO), CB_SETCURSEL, (WPARAM)0, (LPARAM)0);

    // ---- LIST BOX ----

        int list_groupbox_x = 980, list_groupbox_y = 300;
    ...
    // --- PROPERTIES ---
       ...
       int prop_groupbox_x = 1200, prop_groupbox_y = 124;
        ...
        SendDlgItemMessage(_hwnd, IDS_PROPERTIES_LABEL1, WM_SETFONT, (WPARAM)h_font_1, TRUE);
           ...
           // PROPERTIES --- for Flat Visibility - info label and Generate button

                CreateWindowEx(0, L"STATIC", L"Select all cells that PLAYER can move into (using ADD TO MAP button).\nClick GENERATE button to create Flat Visiblity list for all selected cells.", 
                                WS_CHILD | WS_VISIBLE | SS_LEFT, prop_groupbox_x + 10, prop_groupbox_y + 30, 500, 40, _hwnd, 
                                (HMENU)IDS_PROP_GENERATE_INFO_LABEL, GetModuleHandle(NULL), nullptr);

                CreateWindowEx( 0, L"BUTTON", L"Generate Flat Visibility list", WS_CHILD | WS_VISIBLE,
                                prop_groupbox_x + 10, prop_groupbox_y + 75, 250, 32, _hwnd, 
                                (HMENU)IDB_PROP_GENERATE_FV, GetModuleHandle(NULL), NULL);


                CreateWindowEx(WS_EX_CLIENTEDGE, L"LISTBOX", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, prop_groupbox_x + 10, prop_groupbox_y + 20, 55, 450, _hwnd, (HMENU)IDL_PROP_FV_L1, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(WS_EX_CLIENTEDGE, L"LISTBOX", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, prop_groupbox_x + 75, prop_groupbox_y + 20, 55, 450, _hwnd, (HMENU)IDL_PROP_FV_L2, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(WS_EX_CLIENTEDGE, L"LISTBOX", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, prop_groupbox_x + 140, prop_groupbox_y + 20, 55, 450, _hwnd, (HMENU)IDL_PROP_FV_L3, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(WS_EX_CLIENTEDGE, L"LISTBOX", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, prop_groupbox_x + 205, prop_groupbox_y + 20, 55, 450, _hwnd, (HMENU)IDL_PROP_FV_L4, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(WS_EX_CLIENTEDGE, L"LISTBOX", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, prop_groupbox_x + 270, prop_groupbox_y + 20, 55, 450, _hwnd, (HMENU)IDL_PROP_FV_L5, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(WS_EX_CLIENTEDGE, L"LISTBOX", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, prop_groupbox_x + 335, prop_groupbox_y + 20, 55, 450, _hwnd, (HMENU)IDL_PROP_FV_L6, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(WS_EX_CLIENTEDGE, L"LISTBOX", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, prop_groupbox_x + 400, prop_groupbox_y + 20, 55, 450, _hwnd, (HMENU)IDL_PROP_FV_L7, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(WS_EX_CLIENTEDGE, L"LISTBOX", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, prop_groupbox_x + 465, prop_groupbox_y + 20, 55, 450, _hwnd, (HMENU)IDL_PROP_FV_L8, GetModuleHandle(NULL), nullptr);

                CreateWindowEx(0, L"STATIC", L"0-45", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 15, prop_groupbox_y + 450, 40, 20, _hwnd, (HMENU)IDS_PROP_FV_I1, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"45-90", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 80, prop_groupbox_y + 450, 40, 20, _hwnd, (HMENU)IDS_PROP_FV_I2, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"90-135", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 145, prop_groupbox_y + 450, 40, 20, _hwnd, (HMENU)IDS_PROP_FV_I3, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"135-180", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 210, prop_groupbox_y + 450, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_I4, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"180-225", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 275, prop_groupbox_y + 450, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_I5, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"225-270", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 340, prop_groupbox_y + 450, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_I6, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"270-315", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 402, prop_groupbox_y + 450, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_I7, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"315-360", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 465, prop_groupbox_y + 450, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_I8, GetModuleHandle(NULL), nullptr);

                CreateWindowEx(0, L"STATIC", L"#0", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 15, prop_groupbox_y + 470, 40, 20, _hwnd, (HMENU)IDS_PROP_FV_C1, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"#0", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 80, prop_groupbox_y + 470, 40, 20, _hwnd, (HMENU)IDS_PROP_FV_C2, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"#0", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 145, prop_groupbox_y + 470, 40, 20, _hwnd, (HMENU)IDS_PROP_FV_C3, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"#0", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 210, prop_groupbox_y + 470, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_C4, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"#0", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 275, prop_groupbox_y + 470, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_C5, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"#0", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 340, prop_groupbox_y + 470, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_C6, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"#0", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 402, prop_groupbox_y + 470, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_C7, GetModuleHandle(NULL), nullptr);
                CreateWindowEx(0, L"STATIC", L"#0", WS_CHILD | WS_VISIBLE | SS_CENTER, prop_groupbox_x + 465, prop_groupbox_y + 470, 50, 20, _hwnd, (HMENU)IDS_PROP_FV_C8, GetModuleHandle(NULL), nullptr);

                CreateWindowEx(0, L"BUTTON", L"Show", WS_CHILD | WS_VISIBLE, prop_groupbox_x + 10, prop_groupbox_y + 490, 50, 23, _hwnd, (HMENU)IDB_PROP_FV_SB1, GetModuleHandle(NULL), NULL);
                CreateWindowEx(0, L"BUTTON", L"Show", WS_CHILD | WS_VISIBLE, prop_groupbox_x + 75, prop_groupbox_y + 490, 50, 23, _hwnd, (HMENU)IDB_PROP_FV_SB2, GetModuleHandle(NULL), NULL);
                CreateWindowEx(0, L"BUTTON", L"Show", WS_CHILD | WS_VISIBLE, prop_groupbox_x + 142, prop_groupbox_y + 490, 50, 23, _hwnd, (HMENU)IDB_PROP_FV_SB3, GetModuleHandle(NULL), NULL);
                CreateWindowEx(0, L"BUTTON", L"Show", WS_CHILD | WS_VISIBLE, prop_groupbox_x + 208, prop_groupbox_y + 490, 50, 23, _hwnd, (HMENU)IDB_PROP_FV_SB4, GetModuleHandle(NULL), NULL);
                CreateWindowEx(0, L"BUTTON", L"Show", WS_CHILD | WS_VISIBLE, prop_groupbox_x + 270, prop_groupbox_y + 490, 50, 23, _hwnd, (HMENU)IDB_PROP_FV_SB5, GetModuleHandle(NULL), NULL);
                CreateWindowEx(0, L"BUTTON", L"Show", WS_CHILD | WS_VISIBLE, prop_groupbox_x + 335, prop_groupbox_y + 490, 50, 23, _hwnd, (HMENU)IDB_PROP_FV_SB6, GetModuleHandle(NULL), NULL);
                CreateWindowEx(0, L"BUTTON", L"Show", WS_CHILD | WS_VISIBLE, prop_groupbox_x + 400, prop_groupbox_y + 490, 50, 23, _hwnd, (HMENU)IDB_PROP_FV_SB7, GetModuleHandle(NULL), NULL);
                CreateWindowEx(0, L"BUTTON", L"Show", WS_CHILD | WS_VISIBLE, prop_groupbox_x + 465, prop_groupbox_y + 490, 50, 23, _hwnd, (HMENU)IDB_PROP_FV_SB8, GetModuleHandle(NULL), NULL);

                SendDlgItemMessage(_hwnd, IDB_PROP_GENERATE_FV, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_GENERATE_INFO_LABEL, WM_SETFONT, (WPARAM)h_font_1, TRUE);

                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L1), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L2), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L3), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L4), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L5), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L6), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L7), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L8), WM_SETFONT, (WPARAM)h_font_1, TRUE);

                SendDlgItemMessage(_hwnd, IDS_PROP_FV_I1, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_I2, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_I3, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_I4, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_I5, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_I6, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_I7, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_I8, WM_SETFONT, (WPARAM)h_font_1, TRUE);

                SendDlgItemMessage(_hwnd, IDS_PROP_FV_C1, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_C2, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_C3, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_C4, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_C5, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_C6, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_C7, WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendDlgItemMessage(_hwnd, IDS_PROP_FV_C8, WM_SETFONT, (WPARAM)h_font_1, TRUE);

                SendMessage(GetDlgItem(_hwnd, IDB_PROP_FV_SB1), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDB_PROP_FV_SB2), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDB_PROP_FV_SB3), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDB_PROP_FV_SB4), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDB_PROP_FV_SB5), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDB_PROP_FV_SB6), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDB_PROP_FV_SB7), WM_SETFONT, (WPARAM)h_font_1, TRUE);
                SendMessage(GetDlgItem(_hwnd, IDB_PROP_FV_SB8), WM_SETFONT, (WPARAM)h_font_1, TRUE);
}
...
void GUI_Update_Resources(HWND _hwnd)
{
    ...
    TCHAR t_file_size[16];
    ZeroMemory(t_file_size, sizeof(t_file_size));
    Add_Number_Spaces(sizeof(sLV_Level__File_Only), t_file_size);
    ZeroMemory(output, sizeof(output));
    swprintf_s(output, L"%s Bytes", t_file_size);
    SendMessage(GetDlgItem(_hwnd, IDS_RES_FILEH), WM_SETTEXT, 0, (LPARAM)output);

    ZeroMemory(t_file_size, sizeof(t_file_size));
    Add_Number_Spaces(APP_level_file.fv_list_size, t_file_size);
    ZeroMemory(output, sizeof(output));
    swprintf_s(output, L"%s Bytes", t_file_size);
...
void GUI_Update_List_Box(HWND _hwnd)
{
    ...
    switch (gui_current_layer)
    {
        ...
        case LAYERS_FLATS_VISIBILITY_LAYER:
            SendMessage(GetDlgItem(_hwnd, IDL_LIST_LIST), LB_ADDSTRING, 0, (LPARAM)L"01. Select & Generate");
            SendMessage(GetDlgItem(_hwnd, IDL_LIST_LIST), LB_ADDSTRING, 0, (LPARAM)L"02. View");
            break;
    }
...
void GUI_Update_Properties(HWND _hwnd)
{
    ...
    switch (gui_current_layer)
    {
        ...
        case LAYERS_FLATS_VISIBILITY_LAYER:
            switch (gui_current_list_index)
            {
                case LIST_FV_SELECT_AND_GENERATE:
                case LIST_FV_VIEW:
                    GUI_Update_Properties_Flats_Visibility(_hwnd);
                    break;
            }
            break;
    }
...
void GUI_Update_Properties_Flats_Visibility(HWND _hwnd)
{
    switch (gui_current_list_index)
    {
        case LIST_FV_SELECT_AND_GENERATE:
            ShowWindow(GetDlgItem(_hwnd, IDB_ADD_ADDTOMAP), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_GENERATE_FV), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_GENERATE_INFO_LABEL), TRUE);
            break;

        case LIST_FV_VIEW:
            ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L1), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L2), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L3), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L4), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L5), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L6), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L7), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L8), TRUE);

            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I1), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I2), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I3), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I4), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I5), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I6), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I7), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I8), TRUE);

            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C1), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C2), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C3), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C4), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C5), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C6), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C7), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C8), TRUE);

            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB1), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB2), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB3), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB4), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB5), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB6), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB7), TRUE);
            ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB8), TRUE);

            SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L1), LB_RESETCONTENT, 0, 0);
            SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L2), LB_RESETCONTENT, 0, 0);
            SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L3), LB_RESETCONTENT, 0, 0);
            SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L4), LB_RESETCONTENT, 0, 0);
            SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L5), LB_RESETCONTENT, 0, 0);
            SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L6), LB_RESETCONTENT, 0, 0);
            SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L7), LB_RESETCONTENT, 0, 0);
            SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L8), LB_RESETCONTENT, 0, 0);

            // display lists
            if (APP_level_file.map[selected_cell_index].is_fv_marked)
            {
                std::list<short>::iterator iterator;

                TCHAR tbuf[6], tcount[6];

                for (int slice = 0; slice < 8; slice++)
                {
                    for (iterator = MAP_fv_list[selected_cell_index][slice].begin(); iterator != MAP_fv_list[selected_cell_index][slice].end(); ++iterator)
                    {
                        ZeroMemory(tbuf, sizeof(tbuf));
                        ZeroMemory(tcount, sizeof(tcount));

                        wsprintf(tbuf, L"%d", *iterator);
                        wsprintf(tcount, L"#%d", MAP_fv_list[selected_cell_index][slice].size());

                        switch (slice)
                        {
                            case 0: 
                                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L1), LB_ADDSTRING, 0, (LPARAM)tbuf); 
                                SendMessage(GetDlgItem(_hwnd, IDS_PROP_FV_C1), WM_SETTEXT, 0, (LPARAM)tcount);
                                break;

                            case 1:
                                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L2), LB_ADDSTRING, 0, (LPARAM)tbuf);
                                SendMessage(GetDlgItem(_hwnd, IDS_PROP_FV_C2), WM_SETTEXT, 0, (LPARAM)tcount);
                                break;

                            case 2:
                                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L3), LB_ADDSTRING, 0, (LPARAM)tbuf);
                                SendMessage(GetDlgItem(_hwnd, IDS_PROP_FV_C3), WM_SETTEXT, 0, (LPARAM)tcount);
                                break;

                            case 3:
                                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L4), LB_ADDSTRING, 0, (LPARAM)tbuf);
                                SendMessage(GetDlgItem(_hwnd, IDS_PROP_FV_C4), WM_SETTEXT, 0, (LPARAM)tcount);
                                break;

                            case 4:
                                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L5), LB_ADDSTRING, 0, (LPARAM)tbuf);
                                SendMessage(GetDlgItem(_hwnd, IDS_PROP_FV_C5), WM_SETTEXT, 0, (LPARAM)tcount);
                                break;

                            case 5:
                                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L6), LB_ADDSTRING, 0, (LPARAM)tbuf);
                                SendMessage(GetDlgItem(_hwnd, IDS_PROP_FV_C6), WM_SETTEXT, 0, (LPARAM)tcount);
                                break;

                            case 6:
                                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L7), LB_ADDSTRING, 0, (LPARAM)tbuf);
                                SendMessage(GetDlgItem(_hwnd, IDS_PROP_FV_C7), WM_SETTEXT, 0, (LPARAM)tcount);
                                break;

                            case 7:
                                SendMessage(GetDlgItem(_hwnd, IDL_PROP_FV_L8), LB_ADDSTRING, 0, (LPARAM)tbuf);
                                SendMessage(GetDlgItem(_hwnd, IDS_PROP_FV_C8), WM_SETTEXT, 0, (LPARAM)tcount);
                                break;
                        }                        
                    }
                }
...
void GUI_Hide_Properties(HWND _hwnd)
{
    ...
    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_GENERATE_FV), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_GENERATE_INFO_LABEL), FALSE);

    ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L1), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L2), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L3), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L4), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L5), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L6), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L7), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDL_PROP_FV_L8), FALSE);

    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I1), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I2), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I3), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I4), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I5), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I6), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I7), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_I8), FALSE);

    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C1), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C2), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C3), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C4), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C5), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C6), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C7), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDS_PROP_FV_C8), FALSE);

    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB1), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB2), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB3), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB4), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB5), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB6), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB7), FALSE);
    ShowWindow(GetDlgItem(_hwnd, IDB_PROP_FV_SB8), FALSE);
}
...
void Map_Update_Colors(void)
{
    switch (gui_current_layer)
    {
        ...
        case LAYERS_FLATS_VISIBILITY_LAYER:
            hbr_active_bg = hbr_flats_visibility_bg;
            hp_active_grid = hp_flats_visibility_grid;
            hp_active_dotted = hp_flats_visibility_dotted;
            break;
    }
}
            ...
            for (int x = 0; x < MAP_LENGTH; x++)
            {
                ...
                if (x >= f_world_left - 1 && x <= f_world_right)
                {
                    ...
                    switch (gui_current_layer)
                    {
                        ...
                        case LAYERS_FLATS_VISIBILITY_LAYER:
                            Map_Draw_Objects_Flats_Visibility(_hdc, x, y, index);
...
void Map_Draw_Objects_Flats_Visibility(HDC _hdc, int _mx, int _my, int _index)
{
    int cx0, cx1, cy0, cy1;
    ...
    if (APP_level_file.map[_index].is_fv_marked != 0)
    {
        RECT r = { cx0, cy0, cx1, cy1 };
        FillRect(_hdc, &r, hbr_map_flats_visibility);
    }
}

void Map_Draw_Objects_Flats_Visibility_List(HDC _hdc)
{
    if (gui_current_layer == LAYERS_FLATS_VISIBILITY_LAYER && gui_current_list_index == LIST_FV_VIEW && gui_selected_fv_list >= 0 && gui_selected_fv_list <= 7)
    {
        std::list<short>::iterator iterator;

        for (iterator = MAP_fv_list[selected_cell_index][gui_selected_fv_list].begin(); iterator != MAP_fv_list[selected_cell_index][gui_selected_fv_list].end(); ++iterator)
        {
            float cell = (float)(*iterator);

            float my = (float)(int)(cell / LV_MAP_LENGTH);
            float mx = (float)(int)(cell - my * LV_MAP_LENGTH);

            int cx0, cx1, cy0, cy1;

            World_To_Screen((float)mx, (float)my, cx0, cy0);
            World_To_Screen((float)mx + 1.0f, (float)my + 1.0f, cx1, cy1);

            RECT r = { cx0, cy0, cx1, cy1 };
            FillRect(_hdc, &r, hbr_map_flats_visibility_list);
        }
...
void Map_Add_Current_Object()
{
    switch (gui_current_layer)
    {
        ...
        case LAYERS_FLATS_VISIBILITY_LAYER:
            ...
            {
                case LIST_FV_SELECT_AND_GENERATE:
                    APP_level_file.map[selected_cell_index].is_fv_marked = 1;
                    break;
            }
            break;
    }
...
void Map_Read_Current_Cell()
{
    ...
    else
    {
        ...
        MAP_working_cell.is_lightmapped = APP_level_file.map[selected_cell_index].is_lightmapped;
        MAP_working_cell.is_fv_marked = APP_level_file.map[selected_cell_index].is_fv_marked;
...
void Map_Clear_Current_Cell()
{
    switch (gui_current_layer)
    {
        ...
        case LAYERS_FLATS_VISIBILITY_LAYER:
            ...
            {
                case LIST_FV_SELECT_AND_GENERATE:
                    APP_level_file.map[selected_cell_index].is_fv_marked = 0;
                    break;
            }
            break;
    }
