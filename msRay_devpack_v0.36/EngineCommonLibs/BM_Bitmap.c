...
// 
//  ------------------------------------------------------------------------
int32 BM_Read_Bitmap_Indexed_TGA(const char* _input_filename, sBM_Bitmap_Indexed* _output_bitmap, sIO_Prefs* _prefs)
{
    FILE* file_in;
...
//  ---------------------------------------------------------------------------------
// 
//  Loads 32bit TGA into already created 32bit buffer
//  and returns:
// 
//       0  when file is NULL
//      -1  when TGA format problem
//      -2  when this is indexed TGA
//     > 0  positive number is OK
// 
//  ------------------------------------------------------------------------
int32 BM_Read_TGA_32bit(const char* _input_filename, u_int32* _output_bitmap, int32 _mem_size, sIO_Prefs* _prefs)
{
    FILE* file_in;
    file_in = fopen(_input_filename, "rb");
    ...
    // If this is indexed TGA exit.
    if (img.color_map_type == 1)
    {
        tga_free_buffers(&img);
        return -2;
    }
    ...
    memcpy(_output_bitmap, img.image_data, _mem_size);

    // Change the color order if needed.    
    // The TGA is stored in BGRA order. If this screen mode is selected dont change anything.

    switch (_prefs->screen_pixel_format)
    {
        case IO_PIXFMT_ARGB32:
        {
            ...
            for (int32 y = 0; y < img.height; y++)
            {
                ...
                for (int32 x = 0; x < img.width; x++)
                {
                    ...
                    // Switch B and R.
                    unsigned char B = (_output_bitmap[index] & 0xFF000000) >> 24;
                    unsigned char G = (_output_bitmap[index] & 0x00FF0000) >> 16;
                    unsigned char R = (_output_bitmap[index] & 0x0000FF00) >> 8;

                    _output_bitmap[index] = (0 << 24) | (R << _prefs->ch1) | (G << _prefs->ch2) | (B << _prefs->ch3);
                }
            }
        }
        break;
        ...
        case IO_PIXFMT_RGBA32:
        {
            ...
            for (int32 y = 0; y < img.height; y++)
            {
                ...
                for (int32 x = 0; x < img.width; x++)
                {
                    ...
                    _output_bitmap[index] = (R << _prefs->ch1) | (G << _prefs->ch2) | (B << _prefs->ch3);
                }
            }
        }
        break;
...
// 
//  ------------------------------------------------------------------------
int32 BM_Read_Texture_RAW(const char* _input_filename, u_int32* _output_color_data_ptr, u_int8* _output_image_data_ptr, sIO_Prefs* _prefs)
{
    FILE* file_in;
    file_in = fopen(_input_filename, "rb");
...
// 
//  ------------------------------------------------------------------------
int32 BM_Read_Lightmaps_RAW_Separate(   const char* _input_filename, u_int8* _output_wall_lm_ptr, u_int8* _output_floor_lm_ptr, u_int8* _output_ceil_lm_ptr,
...
// 
//  ------------------------------------------------------------------------
int32 BM_Read_Lightmaps_RAW_All(const char* _input_filename, u_int8* _output_lightmaps_ptr, int16 _lightmaps_num)
{
    FILE* file_in;
    file_in = fopen(_input_filename, "rb");
...
//  ---------------------------------------------------------------------------------
// 
//  BM_Resize_And_Save_TGA_Bilinear_Interpolation (32 or 24 bit TGA)
// 
//  The result will be saved as 32 bit.
// 
//  ------------------------------------------------------------------------
int32 BM_Resize_And_Save_TGA_Bilinear_Interpolation(const char* _input_filename, const char* _output_filename, u_int32 _new_width, u_int32 _new_height)
{
    ...
    // Return if image is not 32bit or 24bit. Alpha channel will be skipped.
    if (img.pixel_depth != 32 && img.pixel_depth != 24)
    {
        tga_free_buffers(&img);
        return -3;
    }
    ...
    u_int32 output_byte_size = _new_width * _new_height * 4 * sizeof(u_int8);
    u_int8* output_bitmap = (u_int8*)malloc(output_byte_size);
    
    if (output_bitmap == NULL) return -4;


    // STEP 3.
    // Do the bilinear resize algorithm.
    for (u_int32 i = 0; i < _new_height; i++)
    {
        for (u_int32 j = 0; j < _new_width; j++)
        {
            float32 tmp = (float32)(i) / (float32)(_new_height - 1) * (img.height - 1);
            int32 l = (int32)floor(tmp);
            ...
            tmp = (float32)(j) / (float32)(_new_width - 1) * (img.width - 1);

            int32 c = (int)floor(tmp);
            ...
            /* nearby pixels: a[i][j] */
            u_int32 p1 = *((u_int32*)img.image_data + (l * img.width) + c);
            u_int32 p2 = *((u_int32*)img.image_data + (l * img.width) + c + 1);
            u_int32 p3 = *((u_int32*)img.image_data + ((l + 1) * img.width) + c + 1);
            u_int32 p4 = *((u_int32*)img.image_data + ((l + 1) * img.width) + c);

            /* color components */
            unsigned char blue =    (unsigned char) ((unsigned char)p1 * d1 + (unsigned char)p2 * d2 + (unsigned char)p3 * d3 + (unsigned char)p4 * d4);
            unsigned char green =   (unsigned char) ((unsigned char)(p1 >> 8) * d1 + (unsigned char)(p2 >> 8) * d2 + (unsigned char)(p3 >> 8) * d3 + (unsigned char)(p4 >> 8) * d4);
            unsigned char red =     (unsigned char) ((unsigned char)(p1 >> 16) * d1 + (unsigned char)(p2 >> 16) * d2 + (unsigned char)(p3 >> 16) * d3 + (unsigned char)(p4 >> 16) * d4);

            /* new pixel R G B  */
            *((u_int32*)output_bitmap + (i * _new_width) + j) = (red << 16) | (green << 8) | (blue);
        }
    }


    // STEP 4.
    // Lets save rescaled image using TGA library.
    
    // duplicate and update source tga descriptor.
    tga_image output_tga_descriptor = img;
    output_tga_descriptor.width = _new_width;
    output_tga_descriptor.height = _new_height;
    output_tga_descriptor.image_data = output_bitmap;

    tga_write(_output_filename, &output_tga_descriptor);


    // STEP 5.
    // Lets free input ans output image memory.
    tga_free_buffers(&img);
    free(output_bitmap);
...
// 
//  ------------------------------------------------------------------------
int32 BM_Convert_And_Save_Bitmap_Indexed_To_Texture_RAW(const char* _output_filename, sBM_Bitmap_Indexed* _input_bitmap, u_int8 _tex_type, u_int8 _light_mode, u_int8 _light_treshold)
{
...
// 
//  ------------------------------------------------------------------------
int32 BM_Convert_And_Save_Bitmap_Indexed_To_Lightmap_RAW(const char* _output_filename, sBM_Bitmap_Indexed* _input_bitmap, int32 _wall_lm, int32 _flat_lm)
{
