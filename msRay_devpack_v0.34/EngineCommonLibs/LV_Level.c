...
int8 LV_Load_Level_Prepare_Cells(void)
{
	for (int32 i = 0; i < LV_MAP_CELLS_COUNT; i++)
	{
		...
		if (LV_Level__File_Only.map[i].cell_type)
		{
			LV_map[i].wall_texture_id[0] = LV_Level__File_Only.map[i].wall_texture_id[0];
			LV_map[i].wall_texture_id[1] = LV_Level__File_Only.map[i].wall_texture_id[1];
			LV_map[i].wall_texture_id[2] = LV_Level__File_Only.map[i].wall_texture_id[2];
			LV_map[i].wall_texture_id[3] = LV_Level__File_Only.map[i].wall_texture_id[3];
		}

		LV_map[i].flat_texture_id[0] = LV_Level__File_Only.map[i].flat_texture_id[0];
		LV_map[i].flat_texture_id[1] = LV_Level__File_Only.map[i].flat_texture_id[1];
		...
		if (LV_Level__File_Only.map[i].is_lightmapped)
		{
			...
			{			
				...
				#if defined _WIN32
					LV_map[i].wall_lightmap_id[0] = MA_swap_int16(LV_Level__File_Only.map[i].wall_lightmap_id[0]);
					LV_map[i].wall_lightmap_id[1] = MA_swap_int16(LV_Level__File_Only.map[i].wall_lightmap_id[1]);
					LV_map[i].wall_lightmap_id[2] = MA_swap_int16(LV_Level__File_Only.map[i].wall_lightmap_id[2]);
					LV_map[i].wall_lightmap_id[3] = MA_swap_int16(LV_Level__File_Only.map[i].wall_lightmap_id[3]);
				#endif

				#if defined AMIGA
					LV_map[i].wall_lightmap_id[0] = LV_Level__File_Only.map[i].wall_lightmap_id[0];
					LV_map[i].wall_lightmap_id[1] = LV_Level__File_Only.map[i].wall_lightmap_id[1];
					LV_map[i].wall_lightmap_id[2] = LV_Level__File_Only.map[i].wall_lightmap_id[2];
					LV_map[i].wall_lightmap_id[3] = LV_Level__File_Only.map[i].wall_lightmap_id[3];
				#endif					
			}
			...
			#if defined _WIN32
				LV_map[i].flat_lightmap_id[0] = MA_swap_int16(LV_Level__File_Only.map[i].flat_lightmap_id[0]);
				LV_map[i].flat_lightmap_id[1] = MA_swap_int16(LV_Level__File_Only.map[i].flat_lightmap_id[1]);
			#endif

			#if defined AMIGA
				LV_map[i].flat_lightmap_id[0] = LV_Level__File_Only.map[i].flat_lightmap_id[0];
				LV_map[i].flat_lightmap_id[1] = LV_Level__File_Only.map[i].flat_lightmap_id[1];
			#endif
		}
	}
