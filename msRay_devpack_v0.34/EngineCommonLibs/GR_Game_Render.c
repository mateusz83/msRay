...
// -----------------------------------------------------------
#define		GR_INFO_PANEL_HEIGHT_RELATIVE	40
...
void	GR_Update_Input(void)
{
	...
	if (player_moved)
	{
		...
		int32 z_accumulation_index = (int32)(PL_player.z_accumulation + 0.5f);
		if (z_accumulation_index >= PL_Z_LUT_SIZE__SUB1) z_accumulation_index = 0;

		PL_player.z = PL_player.z__LUT[z_accumulation_index];
	}
...
void	GR_Render_Walls(void)
{
	...
	while(ray_x_counter)
	{
		...
		//int8 cell_type = cell->cell_data >> 1;
		...
		u_int8* tex_y_pos = &tmp_stripe__ptr->wall_stripe__ptr[tex_y_start];

		// Move texture pointer to start of correct mipmap.
		texture += tmp_stripe__ptr->mip_map_offset;
