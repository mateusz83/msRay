...
void	EN_Run(int8* _main_loop)
{
	...
	// If Engine State is not 0 we can just use switch(...) for different states of engine
	// that does not require so much speed.

	if (IO_prefs.engine_state == EN_STATE_GAMEPLAY_RUN) GR_Run();
	else
	{
