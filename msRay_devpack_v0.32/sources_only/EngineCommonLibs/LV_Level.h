...
typedef struct 
{
	...
	// Lets also set the endiannes HOW the file was saved. Assume "0" for PC (Little-Endian) and "1" for Amiga (Big-Endian).
	// Lets always save as "1" (for Amiga) - so 16 and 32 bit values must be reversed before saving.
	// When loading on Amiga we gonna avoid additional computations and the level will load faster.
	int8 endiannes;
